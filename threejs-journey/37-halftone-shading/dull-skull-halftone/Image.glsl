 ...
 *
 * This version is a non-commercial remix featuring
 * halftone/comic-style shading and visual modifications.
 *
 * Changes made:
 * - Modified to halftone shading
 *
 * Credit to KΛTUR for the original shader and SDF work.
 */
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if(d<MAX_DIST){ 
        ...
        vec3 materialColor = vec3(1.0, 0.475, 0.302);
        col = materialColor;
        col *= light;

        vec3 halftoneShadowColor = vec3(0.557, 0.098, 0.722);
        vec3 halftoneLightColor =  vec3(0.898, 1.0, 0.878);
        ...
        col = halftone(
            ...
            0.5,                   // Low
            1.5,                   // High
            halftoneLightColor,    // Point color
            n                      // Normal
        );
